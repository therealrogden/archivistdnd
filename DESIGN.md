...
| `/v1/ask` streaming | Stream tokens via MCP progress notifications; tool return value is the final assembled string. Clients that support progress render it live; clients that don't get the full string as a normal (slower) return. |
...
| Tool | Effect |
|---|---|
| `ask_archivist(question, asker_id?, gm_permissions?)` | Wraps `POST /v1/ask`. Default `gm_permissions=false`, `asker_id=null`. Streams tokens to the caller via MCP progress notifications; final return value is the assembled string. Surfaces `monthlyTokensRemaining` / `hourlyTokensRemaining` in the response. |
...
2. **~** Granular resources. The original composite `archivist://session/{id}` has been dissolved into single-responsibility URIs. **Done:** `campaign`, `campaign/stats`, `campaign/links`; `sessions`, `session/{id}`, `session/{id}/beats`, `session/{id}/moments`, `session/{id}/cast-analysis`; `beats`, `beat/{id}`; `moments`, `moment/{id}`; `quests`, `quest/{id}`; `characters`, `character/{id}`; `items`, `item/{id}`; `factions`, `faction/{id}`; `locations`, `location/{id}`; `journals`, `journal/{id}`; `journal-folders`, `journal-folder/{id}`. **To remove:** `archivist://campaigns` (see Architecture → *Why single-campaign* — single-campaign servers don't need a multi-campaign list; campaign discovery belongs in a setup helper, not the running MCP surface). **Not done:** slim-list projection (step 3) and pagination kwargs (step 4) — list resources currently forward the full Archivist API response. **This step also lands the test scaffolding** (`pytest-httpx` wiring in `conftest.py`, fixture directory layout, first scrubbed fixtures) so every subsequent step has a home for its tests.
...
10. **◯** `ask_archivist` tool with MCP progress-notification streaming. Final assembled string in the return value. Token-budget fields surfaced.
    - **Tests:** progress notifications emitted in order during a mocked stream; the final returned string equals the concatenation of streamed chunks; token-budget fields are present in the return; upstream mid-stream error surfaces as an MCP tool error with a correlation ID; client cancellation is honored (no further chunks after cancel).
...
18. **◯** Fixture maintenance tooling — `scripts/record_fixtures.py` (with content-scrub pass) and `scripts/check_fixture_drift.py`. These are operator scripts that keep the fixtures recorded in steps 1–17 aligned with the live API; they do not add new behavior tests.
